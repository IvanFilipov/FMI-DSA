/*
 * Advanced implementation of hash table, using opened hashing strategy -
 * separate chaining. Depends on some STL stuff as described below.
...
namespace dsa {	
template<typename Key, // what is the type of key
         typename Data,// what is the type of data
		 typename Hasher = std::hash<Key>> // what hash function to be used, by default use std::hash
class hash_map {

private:
	// an inner representation of each table element
	// it is a pair of key and data
...
private:	
	...
	static const size_t MAX_CHAIN_SIZE = 16;
public:
	/* object life cycle */
	hash_map() : table(BASE_SIZE) {} // create table with BASE_SIZE # of empty chains
	hash_map(const Hasher& hasher) : // a constructor with custom hash function
...
private:
	/* helpers */
	// using the objects hash function and size
	// return an valid index in the vector
...
public:
	...
	// some in-lined getters
	// returns the number of elements in the chain with index n.
	size_t chain_size(size_t index) const { return table.at(index).size(); }
	// returns the number of all chain
	size_t chain_count() const { return table.size(); }
	// returns the maximum length of each chain
...
Data& hash_map<Key, Data, Hasher>::
	...
	/// there times here, but
	/// reusing other methods is easier...
