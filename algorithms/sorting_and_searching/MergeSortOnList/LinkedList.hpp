...
private:
	...
	node* mergeSort(node* pStart, int size) {
		...
		if (size == 2) {
			...
			if (pStart->data > pStart->pNext->data) {

				node* temp = pStart;
				pStart = pStart->pNext;

				temp->pNext = pStart->pNext;
				pStart->pNext = temp;
			}
		...
		node* pMid = pHalf->pNext;

		pHalf->pNext = NULL;

		pStart = mergeSort(pStart, size / 2);

		pMid = mergeSort(pMid, size - size / 2);

		pStart = merge(pStart, pMid);
...
void LinkedList<T>::sort(){
	...
	pFront = mergeSort(pFront, curSize);

	node* itFast = pFront;

	while (itFast->pNext != nullptr) {
